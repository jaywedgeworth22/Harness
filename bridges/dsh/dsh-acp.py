...
Stdout is JSON-RPC only.  Drives `dsh --profile <profile>`.  Default profile
is `dsh-headless` (renamed from `headless` 2026-09-19; the new name aligns
with the Harness repo's `dsh` + `mmh` harness matrix).  Override via
DSH_PROFILE env or `--profile` argv.  Auth comes from the process env or
~/.dsh/.credentials.yaml — never from agents.json.
...
DSH_PROFILE = os.environ.get("DSH_PROFILE", "dsh-headless")
